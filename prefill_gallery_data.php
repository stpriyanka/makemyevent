...
try {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM content_sections WHERE section_name = 'gallery'");
    $stmt->execute();
    $exists = $stmt->fetchColumn() > 0;
    
    if ($exists) {
        // Update existing gallery data
        $stmt = $pdo->prepare("UPDATE content_sections SET content = ?, updated_at = CURRENT_TIMESTAMP WHERE section_name = 'gallery'");
        $stmt->execute([json_encode($galleryContent)]);
        echo "Gallery data updated successfully!\n";
    } else {
        // Insert new gallery data
        $stmt = $pdo->prepare("INSERT INTO content_sections (section_name, content, updated_at) VALUES ('gallery', ?, CURRENT_TIMESTAMP)");
        $stmt->execute([json_encode($galleryContent)]);
